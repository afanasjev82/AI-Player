...
public class GenericOpenAIClient implements LLMClient {
    ...
    public GenericOpenAIClient(String apiKey, String modelName, String baseUrl, Duration requestTimeout) {
        this.apiKey = apiKey;
    ...
    private String sendPromptInternal(String systemPrompt, String userPrompt, boolean jsonMode) {
        ...
        try {
            ApiResponse primaryResponse = sendRequest(primaryEndpoint, systemPrompt, userPrompt, jsonMode);
            ...
            ApiResponse alternateResponse = sendRequest(alternateEndpoint, systemPrompt, userPrompt, jsonMode);
    ...
    private ApiResponse sendRequest(ApiEndpoint endpoint, String systemPrompt, String userPrompt, boolean jsonMode)
            throws IOException, InterruptedException {
        JsonObject requestBody = endpoint == ApiEndpoint.RESPONSES
                ? createResponsesRequest(systemPrompt, userPrompt)
                : createChatCompletionsRequest(systemPrompt, userPrompt, jsonMode);
    ...
    private JsonObject createChatCompletionsRequest(String systemPrompt, String userPrompt, boolean jsonMode) {
        ...
        if (jsonMode) {
            ...
            // standard; `format` is Ollama's native field (harmlessly ignored
            // by other OpenAI-compatible servers). Deterministic sampling keeps
            // the emitted array parseable.
